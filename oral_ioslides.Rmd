...
## Sampling Process - 2 {#font-color .build}
...
## Sampling Process - 3 {#font-color .build}
...
3. If we used sample mean as our population mean estimator, it will definitely be <b>overestimated</b>! 
...
## Weighted Distribution {#font-color .smaller .build}
<center>
...
$${f}^{\ast}(x)=\frac{w(x)f(x)}{{E}_{f}(w(x))}.$$ </li>
  <li class="to-build">
  Cox (1962) also proposed the <b>Harmonic Mean</b> ($\frac{n}{\sum_{i=1}^{n}\frac{1}{{x}_{i}}}$) as an estimator of population mean of $X$, and proved that it will converge to $\mu={E}_{f}(x)$ as $n \to \infty$ when $w(x)=x$. </li>
<ol>
...
## Simulation Study - Area (Overview) {#font-color .build}
...
<ol start="4" class="build">
  <li class="to-build"> For each set of samples, calculate the candidate estimators: <b>Arithmetic Mean (AM)</b>, <b>Weighted Mean (WM)</b> and <b>Maximum Likelihood Estimator (MLE)</b>.
  <li class="to-build"> Repeat 3. 4. for the set $Repeated\;Times$ for each $Ratio$. </li>
  <li class="to-build"> Calculate the <b>Mean</b>, <b>Standard Deviation</b> and <b>Root MSE</b> for each candidate estimator. <br />
...
>  1. <b>Middle part</b> of the muscle fiber cell have larger Area, Perimeter and Circularity. <br />
  <ul>
      <li> Area: $\underline{M > P} > D$ </ui> 
      <li> Perimeter: $\underline{M > P} > D$ </li>
      <li> Circularity: $\underline{M > P}>D$  &  $M > \underline{P > D}$ </li>
      <li> Aspect Ratio: $\underline{M > P > D}$ </li>
  </ul>
    <br />
>  2. The appropriate estimator for the size-biased data is <b>Non-parametric Weighted Mean</b>. <br /><br />
>  3. Suggest to use <b>Sampling With Replacement (SWR)</b> rather than Sampling Without Replacement (SWOR) in their future sampling scheme. <br />
  Weighted Mean does not preform well in the case of SWOR when Ratio of $N$ and $n$ is larger than $10\%$.</font>

## Future Work {#font-color .build}
